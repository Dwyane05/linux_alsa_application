...
#define DBG_EN		0
#if(DBG_EN == 1)
#define printf_dbg(x,arg...) printf("[volume_debug]"x,##arg)
#else
#define printf_dbg(x,arg...)
#endif

#define printf_info(x,arg...) printf("[volume_info]"x,##arg)
#define printf_warn(x,arg...) printf("[volume_warn]"x,##arg)
#define printf_err(x,arg...) printf("[volume_err]"x,##arg)
...
static void show_control_id(snd_ctl_elem_id_t *id)
{
	...
	control_iface(id);
#else
	printf_dbg("numid=%u,iface=%s,name='%s'",
			...
			int level)
{
	...
	control_access(info);
#else
	printf_dbg("%s; type=%s,access=%s,values=%i", space, control_type(info), control_access(info), count);
#endif
	switch (type) {
